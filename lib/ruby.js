...
module.exports = class ルビ {
  constructor(元のテキスト = '', kuromoji解析json = [], オプション) {
    ...
    if (this.option.グループ === '--html') {
      this.始まり = '<ruby><rb>'
      if (this.option.use_rp) {
        this.左括弧 = '</rb><rp>' + this.option.左括弧.replace('<', '&lt;').replace('>', '&gt;') + '</rp><rt>'
        this.右括弧 = '</rt><rp>' + this.option.右括弧.replace('<', '&lt;').replace('>', '&gt;') + '</rp></ruby>'
      } else {
        this.左括弧 = '</rb><rt>'
        this.右括弧 = '</rt></ruby>'
      }
      ...
      this.始まり = ''
      this.左括弧 = this.option.左括弧
      this.右括弧 = this.option.右括弧
    }
    this.ルビ除外漢字表現作成()
    this.ルビ付き = ''
    this.ルビ処理()
