...
const オプション解析 = require('./オプションと標準入力').オプション
...
const 標準入力 = require('./オプションと標準入力').標準入力
...
function コマンドグループの判定(入力) {
  ...
  if (オプション.html) {
    ...
    オプション.グループ = '--md-html'
  } else if (たぶんhtml.test(入力.trim())) { // 自動判別: タグの有無で判断
    オプション.グループ = '--html'
    オプション.未指定 ? オプション.未指定.push('--html') : オプション.未指定 = ['--html']
  } else {
    オプション.グループ = '--text'
    オプション.未指定 ? オプション.未指定.push('--text') : オプション.未指定 = ['--text']
  }
}
...
function オプション整理検証() {
  ...
  // 不要なもの
  for (let key of Object.keys(オプション)) {
    if (オプション[key] === undefined) {
      delete オプション[key]
    }
  }
}
...
function brackets検証() { // stringに、空文字ok
  if (オプション.brackets) {
    ...
    オプション.左括弧 = '('
    オプション.右括弧 = ')'
    オプション.未指定 ? オプション.未指定.push('--brackets') : オプション.未指定 = ['--brackets']
  }
}
...
function grade検証() { // 0-7に
  if (オプション.grade) {
    ...
    オプション.grade = 0
    オプション.未指定 ? オプション.未指定.push('--grade') : オプション.未指定 = ['--grade']
  }
}
...
function granularity検証() { // 0-2に
  if (オプション.granularity) {
    ...
    オプション.granularity = 0
    オプション.未指定 ? オプション.未指定.push('--granularity') : オプション.未指定 = ['--granularity']
  }
}
...
function selector検証() { // 配列複数なら,区切りのstringに, 空文字ok
  if (オプション.selector) {
    ...
    オプション.selector = 'body'
    オプション.未指定 ? オプション.未指定.push('--selector') : オプション.未指定 = ['--selector']
  }
}
...
function not_selector検証() { // 配列複数なら,区切りのstringに, 空文字ok
  if (オプション.not_selector) {
    ...
    オプション.未指定 ? オプション.未指定.push('--not-selector') : オプション.未指定 = ['--not-selector']
  }
}
...
function ng_elements検証() { // 配列複数なら|区切りのstringに, 空文字ok
  if (オプション.ng_elements) {
    ...
    オプション.未指定 ? オプション.未指定.push('--ng-elements') : オプション.未指定 = ['--ng-elements']
  }
}
...
function title検証() { // undefinedかstringに, 空文字ok
  if (オプション.title) {
    ...
    オプション.未指定 ? オプション.未指定.push('--title') : オプション.未指定 = ['--title']
  }
...
function headline検証() { // undefinedかstringに, 空文字ok
  if (オプション.headline) {
    ...
    オプション.未指定 ? オプション.未指定.push('--headline') : オプション.未指定 = ['--headline']
  }
}
...
function css検証() { // nullかstringに、空文字ok
  if (オプション.css) {
    ...
    if (オプション.css[0] === true) { // 引数省略: デフォルトstyleに
      オプション.引数省略 ? オプション.引数省略.push('--css') : オプション.引数省略 = ['--css']
      if (オプション.グループ === '--add-class') {
    ...
    オプション.css = null
    オプション.未指定 ? オプション.未指定.push('--css') : オプション.未指定 = ['--css']
    if (オプション.ruby_size) {
      オプション.css = 'ruby>rp,ruby>rt{font-size:' + オプション.ruby_size + 'em;}'
    }
  }
...
function add_class検証() { // stringに
  ...
  if (オプション.add_class[0] === true) { // 引数省略の場合
    オプション.add_class = '学年'
    オプション.引数省略 ? オプション.引数省略.push('--add-class') : オプション.引数省略 = ['--add-class']
  } else {
...
function ruby_size検証() { // Number 1.0-2.0
  if (オプション.ruby_size) {
    ...
    if (オプション.ruby_size[0] === true) { // 引数省略: デフォルト0.5
      オプション.ruby_size = 0.5
      オプション.引数省略 ? オプション.引数省略.push('--ruby-size') : オプション.引数省略 = ['--ruby-size']
    } else {
    ...
    オプション.未指定 ? オプション.未指定.push('--ruby-size') : オプション.未指定 = ['--ruby-size']
  }
}
...
function katakana検証() { // trueかfalse
  if (オプション.katakana) {
    ...
    オプション.未指定 ? オプション.未指定.push('--katakana') : オプション.未指定 = ['--katakana']
  }
}
...
function use_rp検証() { // trueかfalse
  if (オプション.use_rp) {
    ...
    オプション.use_rp = false
    オプション.未指定 ? オプション.未指定.push('--use_rp') : オプション.未指定 = ['--use_rp']
  }
}
...
`
    ...
    オプション.未指定 ? オプション.未指定.push('--comment') : オプション.未指定 = ['--coment']
  }
}
